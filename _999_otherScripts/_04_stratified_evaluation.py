...
def main(merge_type, days_to_consider = 1):
    ...
    y_pred_grp, y_prob_grp = _myFunctions.test_model_ROCKET(rocket_model, X)
    acc_r, _, _, _, f1_r = _myFunctions.calculate_metrics(y, y_pred_grp, y_prob_grp)
    ...
    for group, group_df in df_merged.groupby("merged_PN"):
        ...
        y_pred_grp, y_prob_grp = _myFunctions.test_model_ROCKET(rocket_model, X_group)
        acc_grp, _, _, _, f1_grp = _myFunctions.calculate_metrics(y_group, y_pred_grp, y_prob_grp)
        performance_results.append({
            "Model": "ROCKET",
            "Stratum": group,
            "Accuracy": acc_grp,
    ...
    acc_l, _, _, _, f1_l = _myFunctions.calculate_metrics(y, y_pred_l, y_prob_l)
    performance_results.append({
        "Model": "LSTMFCN",
        "Stratum": "Overall",
        "Accuracy": acc_l,
    ...
    acc_c, _, _, _, f1_c = _myFunctions.calculate_metrics(y, y_pred_c, y_prob_c)
    performance_results.append({
        "Model": "ConvTran",
        "Stratum": "Overall",
        "Accuracy": acc_c,
