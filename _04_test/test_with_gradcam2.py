...
def process_day(day):
    ...
    for model_name in MODELS_TO_RUN:
        ...
        for clsid, info in model_aggregated.items():
            ...
            plt.plot(hours, avg_norm, linewidth=1.5)
            # shaded std area
            plt.fill_between(hours, np.clip(avg_norm - std_norm, 0, 1), np.clip(avg_norm + std_norm, 0, 1), alpha=0.25)
            plt.grid(alpha=0.25)
            plt.xlabel('Time (hours)')
            plt.ylabel('Normalized activation (0-1)')

            # set nicer x-ticks: show ticks every 1 hour (i.e. every 4 frames), but include finer ticks
            if L <= 40:
                # small series: show all quarter-hour ticks
                step = 1
            else:
                # larger series: major tick every 1h (4 frames)
                step = 4
            xtick_positions = np.arange(0, L, step) * ACQUISITION_STEP_HOURS
            xtick_labels = [f"{pos:.2f}h" for pos in (xtick_positions)]
            plt.xticks(xtick_positions, xtick_labels, rotation=45)

            title = f"{model_name} - Grad-CAM average for class {clsid} ({'blasto' if clsid=='1' else 'no_blasto'}) — {n_maps} maps"
            plt.title(title)
            plt.tight_layout()
