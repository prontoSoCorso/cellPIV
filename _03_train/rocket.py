...
import wandb
import orjson  # Per il salvataggio del modello
...
def prepare_data_loaders(df, test_size=0.3):
    ...
    X_train, X_test, y_train, y_test = train_test_split(X, y, test_size=test_size, random_state=conf.seed)
...
if __name__ == '__main__':
    ...
    X_train, X_test, y_train, y_test = prepare_data_loaders(df)
    ...
    # Valutazione del modello su train, validation e test set
    ...
    model_save_path = os.path.join(parent_dir, "_04_test" ,"rocket_classifier_model.pkl")
    orjson.dumps(model, model_save_path)
